...
import { createSuite } from "@/actions/suites/create-suite"
...
const allPeriods: Period[] = ['TWO_HOURS', 'FOUR_HOURS', 'TWELVE_HOURS', 'OVERNIGHT']
...
export function NewSuiteForm() {
  ...
  const { handleSubmit, formState, reset, clearErrors, control, setError, watch, setValue } = form;
  ...
  async function onSubmit(values: NewSuiteFormData) {
    try {
      ...
      const response = await createSuite(values);
                ...
                {allPeriods.map((period, index) => (
                  <div key={period} className="grid gap-2">
                    <FormField control={control} name={`prices.${index}.price`} render={({ field }) => (
                      <FormItem>
                        <FormLabel>{periodLabels[period]}</FormLabel>
