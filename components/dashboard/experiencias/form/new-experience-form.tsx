      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <div className="relative flex aspect-video items-center justify-center rounded-lg border-2 border-dashed border-border bg-secondary/50">
            {imageFile ? (
              <Image src={URL.createObjectURL(imageFile)} alt="Preview" className="object-cover" fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
            ) : (
                <div className="text-center">
                  <Upload className="mx-auto size-8 text-muted-foreground" />
                  <p className="mt-2 text-sm text-muted-foreground">Arraste uma imagem aqui</p>
                </div>
            )}
          </div>
          <Input
